  ...
  addMockPost: publicProcedure.posts.addMockPost.handler(async ({ input, errors, context }) => {
    try {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const seed = uuidv7();
        const picsum = `https://picsum.photos/seed/${seed}/${w}/${h}`;
        ...
        ) as z.infer<typeof postSchema>["material"];
        ...
        ) as z.infer<typeof postSchema>["colour"];
        const garmentType = GARMENT_TYPES[Math.floor(Math.random() * GARMENT_TYPES.length)] as z.infer<typeof postSchema>["garmentType"];
        const size = SIZES[Math.floor(Math.random() * SIZES.length)] as z.infer<typeof postSchema>["size"];
        const containPrice = Math.random() > 0.5;
        const price = containPrice ? Math.floor(Math.random() * 100) + 1 : undefined;

        const randomPostData: z.infer<typeof postSchema> = {
          _id: uuidv7(),
          title: `Post ${i + 1}`,
          ...
          images: Array.from({ length: 4 }).map(() => picsum),
          hashtags: [],
          thumbnailHeight: +h,
          thumbnailWidth: +w,
          archived: false,
